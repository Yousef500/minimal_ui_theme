...
const DeadActiveFilters = ({ handleFiltersReset }) => {
  ...
  return (
    ...
    >
      ...
      />
      <FilterChip
        label={`${t('common.order')}: ${filters.orderby === '0' ? t('common.asc') : t('common.desc')}`}
        name="orderby"
        onDelete={handleDeleteFilter}
      ...
      />
      <FilterChip
        label={`${t('ekram.dead.deathDateFrom')}: ${filters.dateOfDeathFrom?.split('T')[0]}`}
      ...
      <FilterChip
        label={`${t('common.active')}: ${filters.isActive ? t('common.yes') : t('common.no')}`}
        name="isActive"
        onDelete={handleDeleteFilter}
